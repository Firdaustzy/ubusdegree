...
<body>
    ...
    <?php
            ...
            <section id="seat">
                <div id="head">
                    <h4>U-Bus Seat Status</h4>
                </div>
                <div id="main">
                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET">
                        <div style="width: 200px; padding: 8px; border: 1px solid #ccc; border-radius: 4px; font-size: 14px;" class="searchBus">
                            <select id="bus-no" name="bus-no" class="busnoInput">
                                <option value="">Select Bus Number</option>
                            <?php
                                $busArr = json_decode($busJson, true);
                                foreach ($busArr as $bus) {
                                    $busNumber = $bus['bus_no'];
                                    echo "<option value='$busNumber'>$busNumber</option>";
                                }
                             ?>
                            </select>
                        <div class="sugg">
